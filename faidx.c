...
char *faidx_fetch_seq(const faidx_t *fai, char *c_name, int p_beg_i, int p_end_i, char **ref_ptr, int *len)
{
	int l;
	char c;
	khiter_t iter;
	...
	l = 0;
	//	if(seq == NULL)
	*ref_ptr = (char*)malloc(p_end_i - p_beg_i + 2);
	//	else
	//		seq = (char*)realloc(seq, (p_end_i - p_beg_i + 2)*sizeof(char));
	if(*ref_ptr == NULL) {
