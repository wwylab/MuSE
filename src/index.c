...
static int get_tid(ti_index_t *idx, const char *ss)
{
	...
	if (k == kh_end(idx->tname)) { // a new target sequence
		...
		if (idx->n == idx->max) {
			...
			idx->index = (kh_i_t **)realloc(idx->index, idx->max * sizeof(void*));
			idx->index2 = (ti_lidx_t *)realloc(idx->index2, idx->max * sizeof(ti_lidx_t));
		}
...
void ti_index_save(const ti_index_t *idx, BGZF *fp)
{
	...
	ignore_result(bgzf_write(fp, "TBI\1", 4));
	if (ti_is_be) {
		uint32_t x = idx->n;
		ignore_result(bgzf_write(fp, bam_swap_endian_4p(&x), 4));
	} else ignore_result(bgzf_write(fp, &idx->n, 4));
	assert(sizeof(ti_conf_t) == 24);
	if (ti_is_be) { // write ti_conf_t;
		uint32_t x[6];
		memcpy(x, &idx->conf, 24);
		for (i = 0; i < 6; ++i) ignore_result(bgzf_write(fp, bam_swap_endian_4p(&x[i]), 4));
	} else ignore_result(bgzf_write(fp, &idx->conf, sizeof(ti_conf_t)));
	{ // write target names
		char **name;
		int32_t l = 0;
		name = (char **)calloc(kh_size(idx->tname), sizeof(void*));
		for (k = kh_begin(idx->tname); k != kh_end(idx->tname); ++k)
			if (kh_exist(idx->tname, k))
				name[kh_value(idx->tname, k)] = (char*)kh_key(idx->tname, k);
		for (i = 0; i < kh_size(idx->tname); ++i)
			l += strlen(name[i]) + 1;
		if (ti_is_be) {ignore_result(bgzf_write(fp, bam_swap_endian_4p(&l), 4));}
		else {ignore_result(bgzf_write(fp, &l, 4));}
		for (i = 0; i < kh_size(idx->tname); ++i)
			ignore_result(bgzf_write(fp, name[i], strlen(name[i]) + 1));
		free(name);
	}
	for (i = 0; i < idx->n; ++i) {
		khash_t(i) *index = idx->index[i];
		...
		if (ti_is_be) { // big endian
			uint32_t x = size;
			ignore_result(bgzf_write(fp, bam_swap_endian_4p(&x), 4));
		} else ignore_result(bgzf_write(fp, &size, 4));
		for (k = kh_begin(index); k != kh_end(index); ++k) {
			if (kh_exist(index, k)) {
				ti_binlist_t *p = &kh_value(index, k);
				if (ti_is_be) { // big endian
					uint32_t x;
					x = kh_key(index, k); ignore_result(bgzf_write(fp, bam_swap_endian_4p(&x), 4));
					x = p->n; ignore_result(bgzf_write(fp, bam_swap_endian_4p(&x), 4));
					for (x = 0; (int)x < p->n; ++x) {
						bam_swap_endian_8p(&p->list[x].u);
						bam_swap_endian_8p(&p->list[x].v);
					}
					ignore_result(bgzf_write(fp, p->list, 16 * p->n));
					for (x = 0; (int)x < p->n; ++x) {
						bam_swap_endian_8p(&p->list[x].u);
						bam_swap_endian_8p(&p->list[x].v);
					}
				} else {
					ignore_result(bgzf_write(fp, &kh_key(index, k), 4));
					ignore_result(bgzf_write(fp, &p->n, 4));
					ignore_result(bgzf_write(fp, p->list, 16 * p->n));
				}
			}
		}
		...
		if (ti_is_be) {
			int x = index2->n;
			ignore_result(bgzf_write(fp, bam_swap_endian_4p(&x), 4));
		} else ignore_result(bgzf_write(fp, &index2->n, 4));
		if (ti_is_be) { // big endian
			int x;
			for (x = 0; (int)x < index2->n; ++x)
				bam_swap_endian_8p(&index2->offset[x]);
			ignore_result(bgzf_write(fp, index2->offset, 8 * index2->n));
			for (x = 0; (int)x < index2->n; ++x)
				bam_swap_endian_8p(&index2->offset[x]);
		} else ignore_result(bgzf_write(fp, index2->offset, 8 * index2->n));
	}
}
...
static ti_index_t *ti_index_load_core(BGZF *fp)
{
	...
	{ // read target names
		...
		int32_t l;
		uint8_t *buf;
		ignore_result(bgzf_read(fp, &l, 4));
		if (ti_is_be) bam_swap_endian_4p(&l);
		buf = (uint8_t *)calloc(l, 1);
		ignore_result(bgzf_read(fp, buf, l));
		str = (kstring_t *)calloc(1, sizeof(kstring_t));
		for (i = j = 0; i < l; ++i) {
			if (buf[i] == 0) {
	...
	for (i = 0; i < idx->n; ++i) {
		...
		// load linear index
		ignore_result(bgzf_read(fp, &index2->n, 4));
		if (ti_is_be) bam_swap_endian_4p(&index2->n);
		index2->m = index2->n;
		index2->offset = (uint64_t*)calloc(index2->m, 8);
		ignore_result(bgzf_read(fp, index2->offset, index2->n * 8));
		if (ti_is_be)
...
const char **ti_seqname(const ti_index_t *idx, int *n)
{
	...
	khint_t k;
	*n = idx->n;
	names = (const char **)calloc(idx->n, sizeof(void*));
	for (k = kh_begin(idx->tname); k < kh_end(idx->tname); ++k)
		if (kh_exist(idx->tname, k))
...
{
	ti_iter_t iter;
	iter = (ti_iter_t)calloc(1, sizeof(struct __ti_iter_t));
	iter->from_first = 1;
	return iter;
}
	...
	while (1) {
		...
		if (iter->curr_off == 0 || iter->curr_off >= iter->off[iter->i].v) { // then jump to the next chunk
			...
			if (iter->i < 0 || iter->off[iter->i].v != iter->off[iter->i+1].u) { // not adjacent chunks; then seek
				ignore_result(bgzf_seek(fp, iter->off[iter->i+1].u, SEEK_SET));
...
tabix_t *ti_open(const char *fn, const char *fnidx)
{
	...
	t = (tabix_t *)calloc(1, sizeof(tabix_t));
	t->fn = strdup(fn);
